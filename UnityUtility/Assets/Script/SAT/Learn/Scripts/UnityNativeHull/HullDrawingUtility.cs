...
namespace UnityNativeHull
{
    ...
    class HullDrawingUtility
    {
        ...
        public static void DrawDebugHull(NativeHull hull, RigidTransform t, DebugHullFlags options = DebugHullFlags.All, Color BaseColor = default)
        {
            ...
            { 
                NativeHalfEdge edge = hull.GetEdge(i);
                NativeHalfEdge twin = hull.GetEdge(i + 1);

                //hull.GetVertex获取局部空间位置，进而用math.transform根据t来转化为世界空间
                float3 edgeVertex1 = math.transform(t, hull.GetVertex(edge.Origin));
                float3 twinVertex1 = math.transform(t, hull.GetVertex(twin.Origin));
