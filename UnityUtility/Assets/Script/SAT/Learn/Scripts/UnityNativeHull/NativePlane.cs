...
namespace UnityNativeHull
{
    ...
    public unsafe struct NativePlane
    {
        ...
        // 为什么这样表示呢？因为中点是已知的，法线也是已知的，点积的几何意义就是投影。
        // 使用如果一个点p在平面上，那么它在法线方向上的投影应该等于Offset；如果点p在平面上方，那么它在法线方向上的投影应该大于Offset；
        // 如果点p在平面下方，那么它在法线方向上的投影应该小于Offset。
        // 这个定理对平面的所有点都成立，所以我们可以用这个方程来描述平面。
        ...
        {
            // 公式：Distance = (Normal ⋅ Point) - Offset
            // 其中，Normal 是平面的法向量，Point 是要计算距离的点，Offset 是平面到原点的距离。
        ...
        {            
            // 对平面的法向量进行旋转变换
            float3 normal = math.mul(t.rot, plane.Normal);
            // 返回平面的位置和法向量经过刚体变换后的平面,新的偏移量 = 旧的偏移量 + 新法向量与刚体平移部分的点积
            return new NativePlane(normal, plane.Offset + math.dot(normal, t.pos));
        }
    }
}
